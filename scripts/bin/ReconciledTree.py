...
class RecEvent:
    ...
    def __str__(self):
        ...
        for k,v in list(self.additionnalInfo.items()):
    ...
    def makeRecXMLstr(self , name, speciesNames):
        ...
        if spe in list(speciesNames.keys()):
...
class ReconciledTree(ete3.TreeNode):
    ...
    def getTreeRecPhyloXMLAux(self , speciesNames ={}, topoOnly = False, featuresToPrint = {}):
        ...
        if not topoOnly:
            L.append("  <eventsRec>")
            for e in self.eventRecs:
                s = e.makeRecXMLstr(self.name, speciesNames)
                if s == "":
                    continue
                L.append("    " +  s)
            L.append("  </eventsRec>")
...
class ReconciledTreeList:
    ...
    def getEventsSummary(self , includeTransferReception  = True , includeTransferDeparture = False , speciesIdFeature = "name" , indexBySpecies=False):
        ...
        if indexBySpecies:
            ...
            for e in list(EventsSummary.keys()):

                for sp in list(tmp.keys()):
                    tmp[sp][e] = 0
