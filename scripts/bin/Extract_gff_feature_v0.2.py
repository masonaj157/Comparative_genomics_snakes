...
parser.add_argument("-d","--description_field",
					type=str,
					default='Note',
					help="attribute to be used for the sequence description. At the moment only 'Note' and 'Product' are supported.")    
...
def parse_GFF_attributes(gff_attribute) :
    ...
    Product = [x[1] for x in split_attributes if x[0] == 'product']
    attributes_list = gff_attributes([ID, Name, Alias, Note,Product])
...
def add_seq_description(entry, gene, exon_start, exon_end, id_info, seq, description_field):
    ...
    elif description_field == 'Product':
        if hasattr(entry.attributes,'Product') and len(entry.attributes.Product) > 0 :
            new_seq = SeqRecord(
                seq[0].seq[exon_start:exon_end],
                id=id_info,
                description=entry.attributes.Product[0]
                )
        else:
            new_seq = SeqRecord(
                seq[0].seq[exon_start:exon_end],
...
def initiate_CDS_record(entry, id_info, gene_gff_list, seq, description_field):
    ...
    if test_for_multiple_gff_matches(seq, entry) :
        ...
        exon_start = entry.start -1
        exon_end = entry.end
        new_seq = add_seq_description(entry, gene, exon_start, exon_end, id_info, seq, description_field )
        if entry.strand == '-':
            rc_seq = new_seq.seq.reverse_complement()
            new_seq.seq = rc_seq
    return(new_seq)
...
def pull_cds(sequence_list, gff_list) :
    ...
    for entry in CDS_gff_list:
        ...
        if id_info != CDS_ID:
            seq = [seq for seq in sequence_list if seq.id == entry.seqid]
            new_seq = initiate_CDS_record(entry, id_info, gene_gff_list, seq, description_field)
            if not isinstance(new_seq, str) :  
                feature_seqs.append(new_seq)
            else:
                print('hey, an NA')  
